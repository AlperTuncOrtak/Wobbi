...
## CRITICAL PROJECT RULES (FOR ALL AGENTS)
...
```tsx
// âœ… CORRECT - Use inline styles or StyleSheet
...
// âŒ INCORRECT - Do not use NativeWind/tailwindcss classes
function MyScreen() {
  return (
...
We will proceed feature by feature. The workflow must be:
- setup auth -> test auth
- create dashboard -> test dashboard
- add payments -> test payments
- (and so on...)

Never jump to the next feature without fully testing and completing the current one.

---

## Strict Feature-by-Feature Workflow

We develop feature by feature sequentially, strictly following the pattern of building and testing before moving on. For example:
- Setup Auth -> Test Auth
- Create Dashboard -> Test Dashboard
- Add Payments -> Test Payments

Do not jump to the next feature without fully implementing and verifying the current one.
