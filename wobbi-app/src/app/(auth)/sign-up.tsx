...
export default function SignUpScreen() {
  ...
  const handleSignUp = async () => {
    ...
    if (createError) {
      setAuthError((createError as any).errors?.[0]?.message || createError.message || "We couldn't create your account.");
      return;
    }
    const { error: sendError } = await signUp.verifications.sendEmailCode();
    if (sendError) {
      setAuthError("We couldn't send your verification code. Please try again.");
      return;
    }
      ...
      >
        ...
        >
          <View className="flex-1 px-6">
            ...
            {/* Header */}
            <Text className="font-poppins-bold text-[28px] text-text-primary mt-2">Create your account</Text>
